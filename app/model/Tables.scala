...
trait Tables {
   ...
   *  @param roleId Database column role_id SqlType(BIGINT)
   *  @param failureCount Database column failure_count SqlType(INT), Default(0)
   *  @param isAvailable Database column is_available SqlType(BIT), Default(true)
   *  @param isDeleted Database column is_deleted SqlType(BIT), Default(false)
   *  @param isPasswordChange Database column is_password_change SqlType(BIT), Default(false)
   *  @param createAt Database column create_at SqlType(TIMESTAMP)
   *  @param updateAt Database column update_at SqlType(TIMESTAMP) */
  case class UserAccountRow(id: Int, email: String, password: String, passwordSalt: String, firstName: String, lastName: String, roleId: Long, failureCount: Int = 0, isAvailable: Boolean = true, isDeleted: Boolean = false, isPasswordChange: Boolean = false, createAt: Option[java.sql.Timestamp], updateAt: Option[java.sql.Timestamp])
  /** GetResult implicit for fetching UserAccountRow objects using plain SQL queries */
  implicit def GetResultUserAccountRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Long], e3: GR[Boolean], e4: GR[Option[java.sql.Timestamp]]): GR[UserAccountRow] = GR{
    prs => import prs._
    UserAccountRow.tupled((<<[Int], <<[String], <<[String], <<[String], <<[String], <<[String], <<[Long], <<[Int], <<[Boolean], <<[Boolean], <<[Boolean], <<?[java.sql.Timestamp], <<?[java.sql.Timestamp]))
  }
  ...
  class UserAccount(_tableTag: Tag) extends Table[UserAccountRow](_tableTag, "user_account") {
    ...
    /** Database column role_id SqlType(BIGINT) */
    val roleId: Rep[Long] = column[Long]("role_id")
    /** Database column failure_count SqlType(INT), Default(0) */
    val failureCount: Rep[Int] = column[Int]("failure_count", O.Default(0))
    /** Database column is_available SqlType(BIT), Default(true) */
    ...
    /** Database column is_password_change SqlType(BIT), Default(false) */
